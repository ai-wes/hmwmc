...
from hpm_v2 import (
    ...
    StructuredStateTable, StructuredStateConfig,
    TypedEventLog, TypedEventLogConfig,
    StateCheckpointBank, StateCheckpointConfig,
)
...
class WorldModelConfig:
    ...
    # v2: explicit world-state, typed event log, and checkpointed temporal state.
    structured_state_config: Optional[StructuredStateConfig] = None
    typed_event_log_config: Optional[TypedEventLogConfig] = None
    state_checkpoint_config: Optional[StateCheckpointConfig] = None
...
class ForwardOutput:
    ...
    structured_state_sequence: Optional[Tensor] = None
    structured_state_memory: Optional[Tensor] = None
    structured_state_holder_logits: Optional[Tensor] = None
    structured_state_tagged_logits: Optional[Tensor] = None
    structured_state_visible_logits: Optional[Tensor] = None
    structured_state_diagnostics: Optional[Dict[str, float]] = None
    typed_event_entries: Optional[Tensor] = None
    typed_event_mask: Optional[Tensor] = None
    typed_event_times: Optional[Tensor] = None
    typed_event_type_ids: Optional[Tensor] = None
    typed_event_diagnostics: Optional[Dict[str, float]] = None
    typed_event_type_logits: Optional[Tensor] = None
    typed_event_prev_holder_logits: Optional[Tensor] = None
    typed_event_next_holder_logits: Optional[Tensor] = None
    typed_event_scores: Optional[Tensor] = None
    ...
    state_checkpoint_diagnostics: Optional[Dict[str, float]] = None
...
class HomeostaticMultimodalWorldModel(nn.Module):
    ...
    def __init__(self, cfg: WorldModelConfig):
        ...
        self._last_structured_state_diagnostics: Optional[Dict[str, float]] = None
        self._last_typed_event_diagnostics: Optional[Dict[str, float]] = None
        self._last_state_checkpoint_diagnostics: Optional[Dict[str, float]] = None
        ...
        self.structured_state: Optional[StructuredStateTable] = None
        if cfg.structured_state_config is not None and cfg.structured_state_config.enabled:
            self.structured_state = StructuredStateTable(cfg.d_model, cfg.structured_state_config)

        self.typed_event_log: Optional[TypedEventLog] = None
        if (
            cfg.typed_event_log_config is not None
            and cfg.typed_event_log_config.enabled
            and cfg.structured_state_config is not None
            and cfg.structured_state_config.enabled
        ):
            self.typed_event_log = TypedEventLog(
                cfg.d_model,
                cfg.structured_state_config.n_entities,
                cfg.typed_event_log_config,
            )

        self.state_checkpoint_bank: Optional[StateCheckpointBank] = None
        if (
            cfg.state_checkpoint_config is not None
            and cfg.state_checkpoint_config.enabled
            and cfg.structured_state_config is not None
            and cfg.structured_state_config.enabled
        ):
            self.state_checkpoint_bank = StateCheckpointBank(
                cfg.d_model,
                cfg.structured_state_config.n_entities,
                cfg.state_checkpoint_config,
            )
    ...
    def forward(
        ...
        *,
        ...
        if self.structured_state is not None:
            structured_output = self.structured_state(sequence)
            structured_state_sequence = structured_output.sequence
            structured_state_memory = structured_output.memory_tokens
            structured_state_holder_logits = structured_output.holder_logits
            structured_state_tagged_logits = structured_output.tagged_logits
            structured_state_visible_logits = structured_output.visible_logits
            structured_state_diagnostics = structured_output.diagnostics
            self._last_structured_state_diagnostics = structured_output.diagnostics

            if self.typed_event_log is not None:
                typed_event_output = self.typed_event_log(sequence, structured_output, z_per_step)
                typed_event_entries = typed_event_output.entries
                typed_event_mask = typed_event_output.mask
                typed_event_times = typed_event_output.times
                typed_event_type_ids = typed_event_output.type_ids
                typed_event_diagnostics = typed_event_output.diagnostics
                typed_event_type_logits = typed_event_output.event_type_logits
                typed_event_prev_holder_logits = typed_event_output.prev_holder_logits
                typed_event_next_holder_logits = typed_event_output.next_holder_logits
                typed_event_scores = typed_event_output.event_scores
                self._last_typed_event_diagnostics = typed_event_output.diagnostics

            if self.state_checkpoint_bank is not None:
                checkpoint_output = self.state_checkpoint_bank(structured_output, typed_event_scores)
                state_checkpoint_entries = checkpoint_output.entries
                state_checkpoint_mask = checkpoint_output.mask
                state_checkpoint_times = checkpoint_output.times
                state_checkpoint_holder_logits = checkpoint_output.holder_logits
                state_checkpoint_diagnostics = checkpoint_output.diagnostics
                self._last_state_checkpoint_diagnostics = checkpoint_output.diagnostics
        ...
        return ForwardOutput(
            ...
            structured_state_sequence=structured_state_sequence,
            structured_state_memory=structured_state_memory,
            structured_state_holder_logits=structured_state_holder_logits,
            structured_state_tagged_logits=structured_state_tagged_logits,
            structured_state_visible_logits=structured_state_visible_logits,
            structured_state_diagnostics=structured_state_diagnostics,
            typed_event_entries=typed_event_entries,
            typed_event_mask=typed_event_mask,
            typed_event_times=typed_event_times,
            typed_event_type_ids=typed_event_type_ids,
            typed_event_diagnostics=typed_event_diagnostics,
            typed_event_type_logits=typed_event_type_logits,
            typed_event_prev_holder_logits=typed_event_prev_holder_logits,
            typed_event_next_holder_logits=typed_event_next_holder_logits,
            typed_event_scores=typed_event_scores,
            state_checkpoint_entries=state_checkpoint_entries,
            state_checkpoint_mask=state_checkpoint_mask,
            state_checkpoint_times=state_checkpoint_times,
            state_checkpoint_holder_logits=state_checkpoint_holder_logits,
            state_checkpoint_diagnostics=state_checkpoint_diagnostics,
        )
