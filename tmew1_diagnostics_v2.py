"""
TMEW-1 diagnostics and false-cue template for the `_v2` stack.

Adds three things on top of tmew1_queries_v2.py without modifying it:
...
from tmew1_train_v2 import WorldConfig, CurriculumTier, _make_world, _render_vision, _render_audio, _render_numeric, _render_text
...
def _build_query_input_v2(
    ...
    if getattr(output, "hpm_sequence", None) is not None:
        aug = torch.cat([aug, output.hpm_sequence.to(aug.dtype)], dim=-1)
    if getattr(output, "structured_state_sequence", None) is not None:
        aug = torch.cat([aug, output.structured_state_sequence.to(aug.dtype)], dim=-1)
    return aug


def _get_retrieval_context_v2(output: Any) -> Dict[str, Optional[Tensor]]:
    entity_state = None
    ...
    return {
        ...
        "structured_state_holder_logits": getattr(output, "structured_state_holder_logits", None),
        "typed_event_entries": getattr(output, "typed_event_entries", None),
        "typed_event_mask": getattr(output, "typed_event_mask", None),
        "typed_event_times": getattr(output, "typed_event_times", None),
        "state_checkpoint_entries": getattr(output, "state_checkpoint_entries", None),
        "state_checkpoint_mask": getattr(output, "state_checkpoint_mask", None),
        "state_checkpoint_times": getattr(output, "state_checkpoint_times", None),
        "state_checkpoint_holder_logits": getattr(output, "state_checkpoint_holder_logits", None),
    }
...
def recall_by_difficulty(
    ...
    for batch in loader:
        ...
        augmented_seq = _build_query_input_v2(
            output,
            batch.get("audio"),
        ...
        if isinstance(query_head, IterativeQueryHead) or getattr(query_head, "uses_structured_context", False):
            ctx = _get_retrieval_context_v2(output)
        ...
        for bi in range(bs):
            ...
            if isinstance(query_head, IterativeQueryHead) or getattr(query_head, "uses_structured_context", False):
                ctx = _get_retrieval_context_v2(output)
                syn_ctx = {
                    k: (v[bi:bi+1] if isinstance(v, torch.Tensor) else v)
